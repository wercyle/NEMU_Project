...
int _open(const char *path, int flags, mode_t mode) {
  return _syscall_(SYS_open,(intptr_t)path,flags,mode);  //return fd
}

int _write(int fd, void *buf, size_t count) {
  return _syscall_(SYS_write,fd,(intptr_t)buf,count);  //return len
}
...
int _read(int fd, void *buf, size_t count) {
  ...
  return _syscall_(SYS_read,fd,(intptr_t)buf,count);   //return inclen
}

off_t _lseek(int fd, off_t offset, int whence) {       //return file_table[fd].open_offset
  return _syscall_(SYS_lseek,fd,offset,whence);
}

int _close(int fd) {
  return _syscall_(SYS_close,fd,0,0);                   //return 0
}



extern char _end;//declare external variables
static char* program_break=&_end;//record the beginning site
