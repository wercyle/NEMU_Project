...
#define nitems (sizeof(items) / sizeof(items[0]))  //11
#define MAX_PAGE ((nitems - 1) / 10)               //1
#define MAX_IDX_LAST_PAGE ((nitems - 1) % 10)      //0
...
int main(int argc, char *argv[], char *envp[]) {
  ...
  printf("s_w:%d s_h:%d\ screen->pixels:%dn",screen->w,screen->h,*screen->pixels);

  font = new BDF_Font(font_fname);
  logo_sf = SDL_LoadBMP("/share/pictures/projectn.bmp");
  assert(logo_sf);
  set_i_max(); //page=0,i_max=9
...
static void draw_text_row(char *s, int r) { //string to pixles(screen)
  r += 3;
  puts(s);
  draw_str(font, 0, r * font->h, s, 0x123456, 0xffffff);
}

static void display_menu(int n) {  //n is i_max
  clear_display();
  SDL_Rect rect = { .x = screen->w - logo_sf->w, .y = 0 };
  SDL_BlitSurface(logo_sf, NULL, screen, &rect);
  //logo_sf to screen
  //screen and logo_sf all is (SDL_Surface*)
