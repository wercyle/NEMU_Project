...
#define PORT_IO_SPACE_MAX 65535//0xff ff,unit=1byte, so there are 0xffff byte for port_io_space_max
...
void add_pio_map(char *name, ioaddr_t addr, uint8_t *space, int len, io_callback_t callback) {
  ...
  //so each device need a IOMap struct,the put in the maps array
...
uint32_t pio_read_common(ioaddr_t addr, int len) {
  ...
  int mapid = find_mapid_by_addr(maps, nr_map, addr);//addr to maps[mapid]
  assert(mapid != -1);
...
void pio_write_common(ioaddr_t addr, uint32_t data, int len) {
  assert(addr + len - 1 < PORT_IO_SPACE_MAX);
  int mapid = find_mapid_by_addr(maps, nr_map, addr);//addr to maps[pid]
    assert(mapid != -1);

  map_write(addr, data, len, &maps[mapid]);
}

/* CPU interface with device */
