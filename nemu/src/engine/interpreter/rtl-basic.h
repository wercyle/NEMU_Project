...
//rtl_name(DecodeExecState *s,rtlreg_t* dest,const rtlreg_t* src1, const rtlreg_t* src2)
...
//rtl_namei(DecodeExecState *s,rtlreg_t* dest,const rtlreg_t* src1, const sword_t imm)
...
static inline def_rtl(lm, rtlreg_t *dest, const rtlreg_t* addr, word_t offset, int len) { //guest

  *dest = vaddr_read(*addr + offset, len);
}

static inline def_rtl(sm, const rtlreg_t* addr, word_t offset, const rtlreg_t* src1, int len) { //guest
  vaddr_write(*addr + offset, *src1, len);
}

static inline def_rtl(lms, rtlreg_t *dest, const rtlreg_t* addr, word_t offset, int len) {  //guest
...
static inline def_rtl(host_lm, rtlreg_t* dest, const void *addr, int len) {  //host
  switch (len) {
...
static inline def_rtl(host_sm, void *addr, const rtlreg_t *src1, int len) {  //host
  switch (len) {
