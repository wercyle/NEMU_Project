...
typedef struct {
  ...
  // we treat ioaddr_t as paddr_t here,but why?
  paddr_t low;   //starting address
  paddr_t high;  //end address
  uint8_t *space;//target space
...
static inline int find_mapid_by_addr(IOMap *maps, int size, paddr_t addr) {
  int i;
  for (i = 0; i < size; i ++) {
    if (map_inside(maps + i, addr)) {//if addr in [maps[i]->low,maps[i]->high]
      difftest_skip_ref();
      return i;
    }
  }
